...
**Agent run phase:** The thin top bar shows version, connection state, status chips, and transcript stats. While a run is active, the **last assistant bubble** shows spinner + phase line until the answer streams in—then the same bubble shows the reply. There are no separate in-thread tool-call bubbles. Details: [docs/agent-run-phase.md](docs/agent-run-phase.md).
