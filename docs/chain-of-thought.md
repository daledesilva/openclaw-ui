...
- **Live overflow:** Streamed reasoning and tool hints are appended as [`ThoughtItem`](src/chatThreadTypes.ts) rows in `recentThoughtsRef` (see `appendLiveThoughtItem` in `App.tsx`). While the in-flight assistant slot has no answer yet, [`AgentChatBubble`](src/components/AgentChatBubble.tsx) derives phase line text and colours from that buffer (aggregated `reasoningChunk` text, last tool hint/result) via [`phaseBubbleDisplayText`](src/utils/agentRunActivity.ts). **View Thought Process** appears when [`traceHasDisplayableContent`](src/utils/recentThoughtsReducer.ts) is true for the buffer (including prose from joined reasoning chunks). Opening the live modal uses structured `thoughtItems` when the buffer is non-empty, otherwise plain text is empty until items arrive.
...
- **Inline UI:** `src/components/AgentChatBubble.tsx` — assistant chrome with optional **View Thought Process** header (caption-sized), optional `phaseFallbackText` + `isThinking` for the in-run slot, body prioritising streamed answer over phase text. User rows use `src/components/UserChatBubble.tsx`.
