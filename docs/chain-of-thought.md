...
- **Error/abortion rows:** When a run ends in gateway `aborted` or `error`, the assistant row renders only a compact pill title with a **Details** link. Clicking **Details** opens the trace modal with `finalDetails` set to the abort/error message text; the modal shows that text as the **last** grey “thought bubble”.
- **Live overflow:** Streamed reasoning and tool hints are appended as [`ThoughtItem`](src/chatThreadTypes.ts) rows in `recentThoughtsRef` (see `appendLiveThoughtItem` in `App.tsx`). While the in-flight assistant slot has no answer text yet, [`AgentChatBubble`](src/components/AgentChatBubble.tsx) uses component-local logic (`isThinking`, optional tool-line text via [`deriveRecentToolSummaryLine`](src/utils/thoughtProcessing.ts) and streamed `messageText`)—**not** shell run state from `App.tsx`. **View Thought Process** appears when [`traceHasDisplayableContent`](src/utils/recentThoughtsReducer.ts) is true for the buffer (including prose from joined reasoning chunks). Opening the live modal uses structured `thoughtItems` when the buffer is non-empty, otherwise plain text is empty until items arrive.
...
- **Reducer / helpers:** `src/utils/recentThoughtsReducer.ts` — `appendThoughtItem` (dedupes consecutive identical tool hints), `traceHasDisplayableContent`, `assistantHistoryRowDisplaysToUser`, `applyAssistantFinalWithThoughtBuffer`, `foldFetchedHistoryToMessages`, `thoughtItemsToModalSegments` (merges adjacent `reasoningChunk` for modal rows), `findLastHistoricalChainOfThought` (tests / helpers for historical trace detection). `src/utils/thoughtProcessing.ts` — [`deriveRecentToolSummaryLine`](src/utils/thoughtProcessing.ts) (last tool hint or result summary for the thinking-line preview).
- **Modal:** [`ChainOfThoughtModal`](src/components/ChainOfThoughtModal.tsx) — renders structured segments; [`formatThoughtItemsForModal`](src/components/ChainOfThoughtModal.tsx) builds the same content as a plain string (tests in [`ChainOfThoughtModal.test.ts`](src/components/ChainOfThoughtModal.test.ts)).
...
- **Modal:** `src/components/ChainOfThoughtModal.tsx` — `content` is either `{ mode: 'structured', thoughtItems, proseReasoning?, finalDetails? }` or `{ mode: 'plain', text }` (legacy assistant `reasoning`). Renders tool/thought trace bubbles as compact gray pills; `sanitizeDisplayText` on each bubble body. If `finalDetails` is present, it is rendered as the **last** bubble.
- **Triggers:** **View Thought Process** on normal assistant traces, and **Details** on `abortion` / `error` assistant rows (paired `thoughtItems`, with `finalDetails` set to the abort/error message text).
