...
| [`src/api/gateway.ts`](../src/api/gateway.ts) | `sendChatMessage(msg, { sessionKey })`, `fetchChatHistory(limit, sessionKeyOverride)`, `fetchGatewaySessionsList()` (`sessions.list`), `fetchGatewayUsageCost()` (`usage.cost`), `getActiveChatSessionKey` on `initGatewayConnection` for event routing. Inbound `type: "event"` frames are delivered as **`GatewayEventFrame`** through **`onEvent`** (after `chat` session filtering); [`useChatMessageThread`](../src/utils/recentThoughtsReducer.ts) interprets `chat` and agent stream shapes for the transcript. |
